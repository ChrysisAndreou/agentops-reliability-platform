...
[![Tests](https://img.shields.io/badge/tests-1567%20passed-brightgreen)](https://github.com/ChrysisAndreou/agentops-reliability-platform)
...
- **Streaming verification (v0.15)**: Real-time claim verification during token generation with 4 strategies (STRICT, THRESHOLD, LENIENT, ACCUMULATING). Intercepts output mid-stream, extracts claims, verifies against retrieved evidence, and flags hallucinations before the full response is delivered. 54 new tests.
- **Alerting integrations (v0.16)**: Production alerting channels — Slack Block Kit (rich formatted alerts), Discord webhook embeds, and SMTP email. 11 alert rules with 4 severity profiles, cooldown-based storm prevention, and CI-friendly exit codes. 62 new tests.
- **OpenTelemetry observability (v0.17)**: OTLP span export for production monitoring — 7 production metrics (verification pass rate, hallucination rate, trace latency, tool success rate, retrieval latency, cost per run, throughput). Integrates with Jaeger, Grafana, Datadog, and any OTLP-compatible backend. 22 new tests.
- **Production Readiness Assessment (v0.18)**: 8-dimension composite scoring engine (verification, safety, tools, response quality, retrieval, latency, memory, multi-agent). 4 readiness tiers (production, staging, development, experimental). CI gate enforcement with pass/fail thresholds. 3 report formats (JSON, Markdown, HTML). CLI: `agentops readiness assess|gate|scenarios`. 44 new tests.
- **Pluggable LLM backends (v0.19)**: Real LLM API integration with OpenAI, Anthropic, and DeepSeek providers. Drop-in RealLLMAgent replaces SimulatedAgent in all benchmarks. Cost tracking, token counting, and health checks. Factory auto-detection from environment variables. 44 new tests.
- **W&B experiment tracking (v0.20)**: MLOps integration with Weights & Biases — experiment tracking, artifact versioning, model registry, hyperparameter sweeps (grid/random/Bayes, 81-combo grid search). Graceful local JSON fallback for CI/CD. CLI: `agentops mlops sweep|track`. 48 new tests.
- **Model Router (v0.21)**: Cost/latency/capability-aware LLM routing across providers. 5 routing strategies (CHEAPEST, FASTEST, CAPABILITY, ROUND_ROBIN, FAILOVER). Budget enforcement with BudgetExceededError. Per-model cost and latency tracking. 48 new tests.
- **Production RAG retrieval (v0.22)**: Multiple chunking strategies (RecursiveCharacterSplitter with separator hierarchy, SemanticChunker with embedding similarity boundaries, paragraph/sentence/fixed). Cross-encoder reranking (MS MARCO MiniLM) and LLM-based reranking for relevance optimization. BEIR-style evaluation with graded relevance judgments: NDCG@k, MRR, Recall@k, Precision@k, MAP, context_relevance, answer_faithfulness. Realistic benchmark corpus (20 docs, 10 queries). 73 new tests.
- **Streaming performance evaluation (v0.23)**: Nanosecond-precision token timing capture (TTFT, ITL P50/P90/P95/P99), tokens-per-second throughput, stall detection and counting, partial-output quality snapshots at 25/50/75% completion, streaming regression testing with directional tolerance thresholds against versioned baselines, 10-query benchmark corpus across 4 response categories (short/medium/long/technical). 37 new tests.
- **Failure mode analysis & taxonomy (v0.24)**: Systematic classification of 33 failure modes across 9 categories — factuality (hallucination, fabrication, contradiction, outdated info), tooling (wrong tool, malformed args, timeout, not found, ignored results, excessive calls), control flow (infinite loops, premature termination, goal drift), context (overflow, loss), security (prompt injection, data exfiltration, privilege escalation), infrastructure (auth errors, rate limits, server errors), quality, performance, coordination. Automated detection with pattern-based heuristics (10 detectors). Root cause analysis with causal chain modeling (8 chains, 6 trigger modes). Failure clustering with impact scoring. Structured FailureAnalysisReport with reliability scoring and prioritized remediation. 66 new tests.
- **Security red-teaming (v0.25)**: Adversarial testing framework with 27 attack techniques across 5 categories — prompt injection (6 modes: direct, indirect, encoding, multi-turn, multi-language, context confusion), jailbreak (6: DAN, roleplay, prefix injection, refusal suppression, token smuggling, multi-prompt), data exfiltration (5: PII extraction, system prompt leak, tool output siphoning, conversation history theft, embedding inversion), tool misuse (6: parameter injection, privilege escalation, chain exploitation, output manipulation, state corruption, unauthorized access), model extraction (4: capability probing, architecture inference, weight extraction, distillation attack). MITRE ATLAS and OWASP LLM Top 10 mapping. 3 intensity profiles (quick/standard/full). RedTeamRunner with SimulatedTargetAgent for CI reproducibility. SecurityEvaluator with composite security score (0-100), per-category risk assessment, pass/fail gating. 94 new tests.
- **Agent A/B testing & canary deployment (v0.26)**: Statistical experiment framework for comparing AI agent variants. Chi-squared test of independence, Fisher's exact test for small samples, Bayesian A/B testing with beta-binomial conjugate model (Monte Carlo posterior sampling), Welch's t-test for continuous metrics, Mann-Whitney U for non-parametric comparison. Deterministic hash-based traffic splitting with reproducible assignment. Canary deployment with staged rollout (configurable traffic percentages), automatic regression detection (metric, error rate, latency spike), custom evaluation gates, and configurable rollback conditions. Structured A/B evaluation reports with markdown formatting. 102 new tests.
|- **Cost economics & token optimization (v0.27)**: Multi-provider pricing catalog covering 20+ models across OpenAI, Anthropic, Cohere, Mistral, Google, Groq, and DeepSeek. Hierarchical cost tracking across call → agent → session → project with soft/hard budget enforcement and BudgetStatus state machine. Cost optimization via prompt caching detection, context pruning, model routing, and tool call bundling. Cost efficiency evaluation with cost-per-task, cost-per-success metrics, Pareto frontier analysis, budget compliance reporting, and cost regression detection with severity levels. 130 new tests.
|- **EU AI Act compliance framework (v0.28)**: Automated risk classification per Articles 5-6 and Annex III of Regulation (EU) 2024/1689. 18 high-risk conformity assessment requirements across 13 categories. Automated Annex IV technical documentation generation. Compliance auditing engine with pass/fail gating, non-conformity tracking, and structured compliance reports with remediation guidance. 12-scenario compliance benchmark covering unacceptable, high, limited, and minimal risk tiers across 6 sectors (healthcare, finance, education, recruitment, infrastructure, general purpose). 70 new tests.
|- **Multi-agent system evaluation framework (v0.29)**: Systematic evaluation of multi-agent coordination quality with a 10-task benchmark spanning 4 difficulty tiers (easy through expert) and 4 coordination patterns (independent, sequential, consensus, fan-in). 6 coordination quality metrics: decomposition quality, routing accuracy, coordination efficiency, conflict resolution, synthesis quality, and load balance. Scalability scoring across difficulty levels. Structured markdown evaluation reports with per-pattern breakdowns and task-level detail. 65 new tests.
...
- [x] Multi-Agent System Evaluation — 10-task coordination benchmark, 6 quality metrics, 4 difficulty tiers, 65 tests (v0.29)
